...
class ProductPage(BasePage):
    ...
    def should_not_be_successful_message(self):
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_TEXT_NAME), 'Success message is presented, but should not be'

    def message_should_disappear(self):
        assert self.is_disappeared(*ProductPageLocators.SUCCESS_TEXT_NAME), 'Success message is not disappearing, but should be'
